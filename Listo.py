...
RUTA_ARCHIVO_CSV = r"C:\Users\semin\BD2\styles.csv"
...
class IndiceInvertido:
    def __init__(self, ruta_csv: str, ruta_stoplist: str, ruta_indice: str, ruta_normas: str):
        ...
        self.entropias_campos = []  # Añadido para almacenar las entropías
        self.pesos_campos = []      # Se calculará dinámicamente
    ...
    def _calcular_pesos_campos(self):
        ...
        try:
            for chunk in pd.read_csv(self.ruta_csv, chunksize=TAMANIO_CHUNK, encoding='utf-8'):
                ...
                # Puedes limitar el número de documentos para acelerar el cálculo
                if total_documentos >= 100000:  # Por ejemplo, usar solo los primeros 100,000 documentos
                    break
        ...
        self.entropias_campos = []  # Reiniciar la lista de entropías
        for idx_campo in range(total_campos):
            frecuencias = np.array(list(terminos_por_campo[idx_campo].values()))
            if frecuencias.sum() == 0:
                self.entropias_campos.append(0)
                continue
            probabilidades = frecuencias / frecuencias.sum()
            entropia = -np.sum(probabilidades * np.log2(probabilidades + 1e-9))
            self.entropias_campos.append(entropia)
        # Normalizar las entropías para que sumen 1 y asignar pesos
        suma_entropias = sum(self.entropias_campos)
        if suma_entropias == 0:
            self.pesos_campos = [0 for _ in self.entropias_campos]
        else:
            self.pesos_campos = [ent / suma_entropias for ent in self.entropias_campos]
    ...
    def graficar_entropias_pesos(self):
        ...
        # Crear figura y ejes
        fig, ax1 = plt.subplots(figsize=(10, 6))  # Tamaño de la figura ajustado
        ...
        ax1.set_xticklabels(campos, rotation=45, ha='right')  # Rotación y alineación de etiquetas
...
if __name__ == "__main__":
    ...
    # Graficar entropías y pesos
    indice.graficar_entropias_pesos()
